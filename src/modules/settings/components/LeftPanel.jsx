...
const COMPONENT_MATERIALS = [
  ...
  { type: "editableTable", label: "可编辑表格", icon: "table", category: "field" },
];

export default function LeftPanel({ designMode }) {
  const handleDragStart = (e, item) => {
    e.dataTransfer.setData("componentParams", JSON.stringify(item));
    window.__DRAGGED_ITEM_TYPE__ = item.type;
  };

  const handleDragEnd = () => {
    window.__DRAGGED_ITEM_TYPE__ = null;
  };
  ...
  const displayFields = designMode === "list" ? fieldItems.filter((f) => !["upload"].includes(f.type)) : fieldItems;
